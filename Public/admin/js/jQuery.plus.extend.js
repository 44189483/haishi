...
jQuery.fn.extend({
	...
	inputSwitch:function(className) {
		return this.each(function(){
			var _this = $(this) ;
			var def = _this.val() ;
			if(_this.val().length != 0) _this.addClass(className)
			_this.focus(function(){
				if(_this.val() == 0) _this.addClass(className)
			}).blur(function(){
				if(_this.val().length == 0) _this.removeClass(className)
			})	
		})
	}
})
...
jQuery.extend({
	//<script>$.flashSwf({path:"path.swf",width:width,height:height})</script>
	flash:function(opt){
		var def = {
			wmode:"transparent"
		}
		var opt = $.extend(def,opt) ;
		var swf = '<object classid="clsid:D27CDB6E-AE6D-11cf-96B8-444553540000" codebase="http://download.macromedia.com/pub/shockwave/cabs/flash/swflash.cab#version=7,0,19,0" width="'+opt.width+'" height="'+opt.height+'">' ;
		swf += '<param name="movie" value="'+opt.path+'">' ;
		swf += '<param name="quality" value="high">' ;
		swf += '<param name="wmode" value="'+opt.wmode+'" />' ;
		swf += '<param name="menu" value="false">'
		swf += '<embed src="'+opt.path+'" quality="high" pluginspage="http://www.macromedia.com/go/getflashplayer" type="application/x-shockwave-flash" width="'+opt.width+'" height="'+opt.height+'" wmode="'+opt.wmode+'"></embed>' ;
		swf += '</object>' ;		
		return document.write(swf) ;
	},
	...
	tab:function(opt){
		...
		var opt = $.extend(def,opt) ;
		var parentElm = $(opt.parent)
		var tabElm = parent.find(opt.tabElm) ;
		var switchElm = parent.find(opt.switchElm) ;
		var trigger = opt.trigger ;
		var tabClassName = opt.className ;
		var tabSwitchClassName = opt.className ;
